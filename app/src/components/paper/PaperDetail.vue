...
import { getPaper, updatePaper, deletePaper } from "@/services/PaperService";
...
export default defineComponent({
  ...
  methods: {
    ...
    async handleUpdate() {
      if (typeof this.$route.params.id === "string") {
        const res = await updatePaper(this.$route.params.id, this.paper);
        this.$router.push("/papers");
      }
    },
    async handleDelete() {
      if (typeof this.$route.params.id === "string") {
        const res = await deletePaper(this.$route.params.id);
        this.$router.push("/papers");
      }
    },
  },
  mounted() {
    if (typeof this.$route.params.id === "string") {
      this.loadPaper(this.$route.params.id);
    }
  },
});
</script>
